<!-- Profile README for Saikiran-Rudra -->

<h1 align="center">Hi 👋, I'm Saikiran Rudra</h1>
<h3 align="center">Aspiring Full-Stack Developer | Learning AI/ML & IoT</h3>

<p align="center">
<img src="https://readme-typing-svg.demolab.com/?lines=👨‍💻%20Solving%20real-life%20problems;🌱%20Learning%20AI%20and%20IoT;🧠%20Leading%20Student%20Developer%20Club;🚀%20Freelancer&center=true&width=440&height=45&font=Fira%20Code&pause=1000&color=F75000" alt="Typing SVG" />
</p>

---

### 👨‍💻 About Me

- 🎓 3rd Year B.Tech Computer Engineering @ Bhagwan Mahavir University, Surat  
- 🥇 Gold Medalist in Diploma CSE (2023)  
- 🧠 Co-Founder of **Student Developer Club @ BMU**  
- 🛠️ Building smart solutions using **AI/ML, IoT**  
- 🌱 Currently learning: Advanced Full-Stack, ML Integration with IoT  
- 💬 Ask me about: Innovation, Projects, or Club Leadership  
- ⚡ Fun fact: I build and fly drones too!

---

### 🛠️ Tech Stack & Projects

**Languages & Frameworks:**
- 💻 **Languages**: C, C++, Python, JavaScript, PHP  
- 🌐 **Frontend**: React.js, HTML5, CSS3, Bootstrap  
- 🖥️ **Backend**: Node.js (learning)  
- 🧠 **ML/AI**: Pandas, Scikit-Learn, Streamlit  
- 📦 **Database**: MySQL, MongoDB  
- 🔌 **Tools**: Git, Figma, Canva, Netlify, VS Code  
- 📡 **IoT**: Arduino, Raspberry Pi, Sensors & Actuators

**Projects:**
- 🔐 **CyberGuardLab Portal** – Cybersecurity awareness platform  
- 💧 **Smart Irrigation System** – IoT + ML-based crop watering solution  
- 📚 **Student CMS Portal** – Academic records management system  
- 💸 **Library Refund Portal** – Automating book return refunds  

---

### 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Saikiran-Rudra&show_icons=true&theme=tokyonight" alt="GitHub stats" />
</p>
